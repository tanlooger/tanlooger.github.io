...
for (i = 0; i < chaps.length; i++) {
  ...
  if(levelprev > level){
    ...
    allchapter.push({slug:['en/'+bookid], id:i+'', parent_id:i?allchapter[i-1].parent_id+'':'', title:chaps[i].trim()})
  }
  ...
  if(nextindex < 0){
    ...
    if (fs.existsSync('en'))fs.rmdirSync('en', { recursive: true, force: true })
    process.exit(1)
  }
